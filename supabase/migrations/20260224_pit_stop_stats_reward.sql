-- Migration: 20260224_pit_stop_stats_reward
-- Description: Extends statistics-based reward system to Pit Stop Challenge.
-- 1. Backfills 'contributes_to_pool' for Pit Stop scores.
-- 2. Updates 'get_monthly_prize_pool' to accept game_type.
-- 3. Updates 'play_pit_stop_game' with the new stats-based logic (Cost/Refund/Profit).
-- 4. Updates 'distribute_monthly_minigame_rewards' to handle both games with STRICT distribution (25,18,15,12,10,8,6,4,2).
-- 5. Updates 'get_monthly_leaderboard' to accept game_type.

-- 1. Backfill Pit Stop Scores
...
-- Pit Stop times are usually 2000-6000ms. Good time < 3500ms?
-- Let's say < 3500ms is a "Good Play" for backfill purposes.
...
-- 2. Update get_monthly_prize_pool (Generic)
...
-- Handles BOTH games with STRICT distribution
...
BEGIN
    ...
    LOOP
        ...
        v_pool_data := get_monthly_prize_pool(p_year, p_month, v_game_type);
...
-- 5. Update get_monthly_leaderboard (Generic)
