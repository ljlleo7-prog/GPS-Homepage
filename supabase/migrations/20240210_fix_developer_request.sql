-- Fix for request_developer_access function return type conflict
-- We must DROP the function before changing its return type from VOID to JSONB

DROP FUNCTION IF EXISTS public.request_developer_access();

-- Recreate request_developer_access with JSONB return
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = v_user_id) THEN
    -- Attempt to create profile if missing (recovery)
...
-- Ensure get_developer_inbox exists and is correct
...
BEGIN
  ...
  -- First ensure the column exists to avoid errors if previous migration failed
