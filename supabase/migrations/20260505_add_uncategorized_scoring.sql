-- Add fallback scoring for uncategorized engagement events
-- Preserves categorized scoring with bonuses and caps, adds 1-point default for everything else
...
BEGIN
  ...
  -- Forum posts with like bonus (fixed GROUP BY)
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY c.author_id ORDER BY c.created_at) as rn
