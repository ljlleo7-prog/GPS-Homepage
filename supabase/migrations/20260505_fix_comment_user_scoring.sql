-- Fix forum comment scorer to use forum_comments.user_id
-- Keeps categorized scoring plus fallback 1-point default for uncategorized engagement events
...
BEGIN
  ...
  WHERE se.period_id = v_period_id
    AND NOT EXISTS (
      ...
      WHERE s.period_id = se.period_id AND s.user_id = se.user_id AND s.status = 'SUSPENDED'
    );

  -- Forum posts with like bonus (max 15/week)
  ...
  SELECT
    ...
    'forum_post',
    id,
    1 + LEAST(like_count, 10) * 2,
    jsonb_build_object('title', title, 'likes', like_count, 'length', content_length)
  FROM (
    SELECT
      p.id,
      p.author_id,
      p.title,
      length(trim(COALESCE(p.content, ''))) as content_length,
      COUNT(DISTINCT l.user_id) FILTER (WHERE l.user_id <> p.author_id) as like_count,
    ...
    GROUP BY p.id, p.author_id, p.title, p.content, p.created_at
  ) posts
  WHERE rn <= 15
  ON CONFLICT (period_id, user_id, event_type, source_type, source_id) DO UPDATE SET
    points = EXCLUDED.points, metadata = EXCLUDED.metadata;

  -- Forum comments (max 30/week)
  INSERT INTO public.community_contribution_score_events (period_id, user_id, event_type, source_type, source_id, points, metadata)
  SELECT
    v_period_id,
    user_id,
    ...
    id,
    1,
    jsonb_build_object('post_id', post_id, 'length', content_length)
  FROM (
    SELECT
      c.id,
      c.user_id,
      c.post_id,
      length(trim(COALESCE(c.content, ''))) as content_length,
      ROW_NUMBER() OVER (PARTITION BY c.user_id ORDER BY c.created_at) as rn
    FROM public.forum_comments c
    LEFT JOIN public.community_contribution_scores s ON s.period_id = v_period_id AND s.user_id = c.user_id
  ...
  -- Daily bonus (max 7/week)
  INSERT INTO public.community_contribution_score_events (period_id, user_id, event_type, source_type, source_id, points, metadata)
  SELECT v_period_id, user_id, 'daily_bonus', source_type, source_id, 1, metadata
  FROM (
  ...
  ON CONFLICT (period_id, user_id, event_type, source_type, source_id) DO UPDATE SET points = EXCLUDED.points, metadata = EXCLUDED.metadata;

  -- Minigames (max 20/week)
  INSERT INTO public.community_contribution_score_events (period_id, user_id, event_type, source_type, source_id, points, metadata)
  SELECT v_period_id, user_id, 'minigame', source_type, source_id, 1, metadata
  FROM (
    SELECT e.*, ROW_NUMBER() OVER (PARTITION BY e.user_id ORDER BY e.created_at) as rn
    FROM public.community_engagement_events e
    LEFT JOIN public.community_contribution_scores s ON s.period_id = v_period_id AND s.user_id = e.user_id
  ...
  -- Poll votes (no strict limit)
  INSERT INTO public.community_contribution_score_events (period_id, user_id, event_type, source_type, source_id, points, metadata)
  SELECT v_period_id, v.user_id, 'poll_vote', 'community_poll', v.poll_id, 1, jsonb_build_object('option_id', v.option_id)
  ...
  -- Market actions (max 15/week)
  INSERT INTO public.community_contribution_score_events (period_id, user_id, event_type, source_type, source_id, points, metadata)
  SELECT v_period_id, user_id, 'market_action', source_type, source_id, 1, metadata
  FROM (
    SELECT e.*, ROW_NUMBER() OVER (PARTITION BY e.user_id ORDER BY e.created_at) as rn
    FROM public.community_engagement_events e
    LEFT JOIN public.community_contribution_scores s ON s.period_id = v_period_id AND s.user_id = e.user_id
  ...
  -- Uncategorized engagement events (fallback: 1 point each, max 50/week)
  INSERT INTO public.community_contribution_score_events (period_id, user_id, event_type, source_type, source_id, points, metadata)
  SELECT v_period_id, user_id, event_type, source_type, source_id, 1, metadata
  FROM (
    SELECT e.*, ROW_NUMBER() OVER (PARTITION BY e.user_id ORDER BY e.created_at) as rn
    FROM public.community_engagement_events e
    LEFT JOIN public.community_contribution_scores s ON s.period_id = v_period_id AND s.user_id = e.user_id
    WHERE e.created_at >= v_period.period_start::timestamptz
      AND e.created_at < v_period.period_end::timestamptz
      AND e.event_type NOT IN ('daily_bonus_claimed', 'minigame_play_completed', 'ticket_listing_created', 'ticket_listing_purchased', 'driver_bet_created', 'driver_bet_ticket_bought')
      AND COALESCE(s.status, 'PENDING') <> 'SUSPENDED'
  ) sub WHERE rn <= 50
  ON CONFLICT (period_id, user_id, event_type, source_type, source_id) DO UPDATE SET points = EXCLUDED.points, metadata = EXCLUDED.metadata;
  ...
  SELECT
    v_period_id,
    se.user_id,
    COALESCE(SUM(se.points) FILTER (WHERE se.event_type = 'forum_comment'), 0)::int,
...
-- Recalculate current period immediately after replacing the function
