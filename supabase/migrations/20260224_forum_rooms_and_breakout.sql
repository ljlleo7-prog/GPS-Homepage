...
CREATE POLICY "Forum rooms are viewable by everyone" 
ON public.forum_rooms FOR SELECT USING (true);

-- Only developers can create rooms
CREATE POLICY "Developers can create rooms" 
ON public.forum_rooms 
FOR INSERT 
WITH CHECK (
  EXISTS (
...
-- Only developers/admins can update rooms
CREATE POLICY "Developers can update rooms" 
ON public.forum_rooms 
FOR UPDATE 
USING (
  EXISTS (
...
-- Insert Default Public Forum
INSERT INTO public.forum_rooms (name, description, is_official)
VALUES ('Public Forum', 'General discussion for everyone.', true)
ON CONFLICT (name) DO NOTHING;

-- Add room_id to forum_posts
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'forum_posts' AND column_name = 'room_id') THEN
        ALTER TABLE public.forum_posts ADD COLUMN room_id UUID REFERENCES public.forum_rooms(id) ON DELETE SET NULL;
    END IF;
END $$;

-- Update existing posts to belong to Public Forum
UPDATE public.forum_posts
SET room_id = (SELECT id FROM public.forum_rooms WHERE name = 'Public Forum')
WHERE room_id IS NULL;

-- Update Resolve Driver Bet Function to NOT feature the post
-- Copying the logic from 20260221_capped_driver_bet_resolution.sql but changing is_featured to false
-- and setting room_id to Public Forum
...
BEGIN
  ...
  -- Get Public Room ID
  SELECT id INTO v_public_room_id FROM public.forum_rooms WHERE name = 'Public Forum';
...
-- Function to create forum room
CREATE OR REPLACE FUNCTION public.create_forum_room(
  p_name TEXT,
  p_description TEXT
)
RETURNS JSONB
...
BEGIN
  ...
  IF NOT v_is_dev THEN
    RETURN jsonb_build_object('success', false, 'message', 'Only developers can create rooms');
  END IF;

  INSERT INTO public.forum_rooms (name, description, created_by, is_public, is_official)
  VALUES (p_name, p_description, v_user_id, true, false)
