-- Fix duplicate ON CONFLICT rows by using engagement event ids as score event sources
...
BEGIN
  ...
  SELECT
    ...
    id,
    1,
    metadata || jsonb_build_object('original_source_type', source_type, 'original_source_id', source_id)
  FROM (
  ...
  SELECT
    ...
    id,
    1,
    metadata || jsonb_build_object('original_event_type', event_type, 'original_source_type', source_type, 'original_source_id', source_id)
  FROM (
    SELECT e.*, ROW_NUMBER() OVER (PARTITION BY e.user_id ORDER BY e.created_at) as rn
    FROM public.community_engagement_events e
    LEFT JOIN public.community_contribution_scores s ON s.period_id = v_period_id AND s.user_id = e.user_id
