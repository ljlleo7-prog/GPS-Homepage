-- Complete Developer Flow Fix
-- 1. Request Access (Robust Fix)
-- 2. Get Inbox (Ensure Existence)
-- 3. Approve Access (Missing)
-- 4. Decline Access (Secure RPC)

-- DROP existing functions to ensure clean slate
DROP FUNCTION IF EXISTS public.request_developer_access();
DROP FUNCTION IF EXISTS public.get_developer_inbox();
DROP FUNCTION IF EXISTS public.approve_developer_access(uuid);
DROP FUNCTION IF EXISTS public.decline_developer_access(uuid);
...
BEGIN
  ...
  -- Check if caller is approved developer
  SELECT (developer_status = 'APPROVED') INTO v_is_dev
  FROM public.profiles
  WHERE id = v_user_id;
...
-- 4. Decline Developer Access
CREATE OR REPLACE FUNCTION public.decline_developer_access(target_user_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
