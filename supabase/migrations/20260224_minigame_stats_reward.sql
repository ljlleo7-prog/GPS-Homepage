-- Migration: 20260224_minigame_stats_reward
-- Description: Implements statistics-based reward system for Minigames (Reaction).
-- 1. Adds 'contributes_to_pool' column to minigame_scores.
-- 2. Updates 'play_reaction_game' with new logic:
--    - Cost: 1 Token (deducted).
--    - Refund: 1 Token if Score < Global Avg (last 7 days) OR Score < User Avg (last 20 plays).
--    - Profit: Bonus tokens based on Z-Score (Global Stats) if cooldown passed.
--    - Pool Contribution: Only "Good Plays" (Score < Global Avg) contribute to the monthly pool.
-- 3. Updates 'get_monthly_prize_pool' to count only contributing plays.
-- 4. Updates 'distribute_monthly_minigame_rewards' to rank by AVG(Top 10 Scores) and require min 10 plays.
-- 5. Updates 'get_monthly_leaderboard' to reflect the new ranking logic (AVG Top 10).

-- 1. Add Column
ALTER TABLE public.minigame_scores 
ADD COLUMN IF NOT EXISTS contributes_to_pool BOOLEAN DEFAULT false;

-- Update existing scores to contribute (legacy support)
-- Set existing scores < 400ms to true
UPDATE public.minigame_scores 
SET contributes_to_pool = (score_ms < 400) 
WHERE contributes_to_pool IS FALSE; 

-- 2. Update play_reaction_game
...
-- 3. Update get_monthly_prize_pool
-- Only counts plays where contributes_to_pool is true
...
-- 4. Update distribute_monthly_minigame_rewards
-- Rank by AVG(Top 10 Scores), Minimum 10 plays
...
BEGIN
    ...
    -- Logic: Top 10 users by AVG of their best 10 scores
    ...
    LOOP
        ...
        -- 1st: 30%, 2nd: 20%, 3rd: 10%, 4th-10th: Share 40% (approx 5.7% each)
        IF v_leaderboard.rank = 1 THEN
            v_reward := FLOOR(v_total_pool * 0.30);
        ELSIF v_leaderboard.rank = 2 THEN
            v_reward := FLOOR(v_total_pool * 0.20);
        ELSIF v_leaderboard.rank = 3 THEN
            v_reward := FLOOR(v_total_pool * 0.10);
        ELSE
            v_reward := FLOOR(v_total_pool * 0.40 / 7.0);
        END IF;
        ...
        IF v_reward > 0 THEN
            ...
            SELECT 
                ...
                'Minigame Monthly Reward: Rank #' || v_leaderboard.rank || ' (Avg: ' || ROUND(v_leaderboard.avg_best_10, 1) || 'ms)'
            FROM public.wallets 
            WHERE user_id = v_leaderboard.user_id;
...
-- 5. Update get_monthly_leaderboard
-- Returns AVG(Top 10) as best_score, filters < 10 plays
...
RETURNS TABLE (
    user_id UUID,
    username TEXT,
    best_score INTEGER, -- Represents AVG of Top 10
    rank BIGINT,
    total_plays BIGINT,
